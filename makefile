...
$(GOLANGCI_LINT_MODULES): .tools/.custom-gcl.yml $(GOLANGCI_LINT) $(shell find checks lib plugin -name '*.go' -not -path '*/testdata/*')
...
lint: $(GOLANGCI_LINT_MODULES) ## Check source code with the golangci linters (incl. this repo's own AZG rules)
