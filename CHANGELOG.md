...
- `AZG008`: no suggested fix when the dereferenced value is sent as a PUT/PATCH/POST body — the callee serialises that parameter while performing a write, derived by the new `writebody` fact analyzer from `net/http` method constants and `encoding/json`/`xml` marshalling alone (closures and delegating wrappers included, no SDK names) — directly or via a local; `pointer.From` there would send an empty write request, so the report stands and names the reason ([#36](https://github.com/katbyte/azproviderlint/pull/36))
