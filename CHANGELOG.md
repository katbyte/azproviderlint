...
- `AZG008`: `pointer.To*` and the `flag` constructors are no longer special-cased as non-nil sources — their bodies prove it through `nilguard_returns` like any function; request-body flow detection moved into `requestbody` (`Sinks`, `SentAsBody`) and runs once per function; `lib/nilguard` split into files by concern; no behaviour change ([#38](https://github.com/katbyte/azproviderlint/pull/38))
