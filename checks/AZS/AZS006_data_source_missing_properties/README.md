...
AZS006 pairs each data source with the resource of the same name and reports resource properties the data source does not expose.
...
Pairing works the same way as in AZS005 across untyped, typed, and framework registrations. The schema is read per style: the untyped resource function's body, the typed `Arguments()` and `Attributes()` methods, or the framework `Schema()` method. Property names are collected all the way down, and helper functions in the same package that return schema pieces are followed.
...
For providers whose policy is to keep secrets out of data sources across the board. Set with `-AZS006.<option>` on the CLI or under the rule name in the golangci settings; see the [root README](../../../README.md#options).
...
To silence every report for a data source, put `//azignore:AZS006 - <reason>` on its registration line, or on the line above it. The reason is required.
