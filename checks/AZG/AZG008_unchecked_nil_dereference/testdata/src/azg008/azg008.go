...
func validClosureParam(d data) {
	f := func(count *int) { d.Set("count", *count) }
	f(nil)
}

// Should NOT be flagged: the ok companion is checked on the left of && in the same condition.
func validOkInCondition(lookup func() (*int, bool)) bool {
	if count, ok := lookup(); ok && *count > 0 {
		return true
	}
	return false
}
...
func invalidOkOrInCondition(lookup func() (*int, bool)) bool {
	count, ok := lookup()
	return ok || *count > 0 // want "dereference of possibly-nil `count` may panic - add a nil check or use pointer.From"
}
