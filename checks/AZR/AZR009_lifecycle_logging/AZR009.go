...
func run(pass *analysis.Pass) (any, error) {
	...
	for file, findings := range byFile {
		...
		for i, f := range findings {
			edits := []analysis.TextEdit{deleteLine(pass, f.stmt)}
...
func deleteLogImport(pass *analysis.Pass, file *ast.File) *analysis.TextEdit {
	for _, decl := range file.Decls {
		...
		for _, spec := range gen.Specs {
			...
			if len(gen.Specs) == 1 {
				e := deleteLine(pass, gen)
				return &e
			}
			e := deleteLine(pass, imp)
			return &e
		}
	}
	return nil
...
// comment included), so no blank line is left behind; when other code shares the line only
// the node itself goes.
func deleteLine(pass *analysis.Pass, node ast.Node) analysis.TextEdit {
	...
	if tf != nil {
		...
		if content, err := pass.ReadFile(tf.Name()); err == nil {
			...
			// a trailing comment belongs to the statement and goes with it
